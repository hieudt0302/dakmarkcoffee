<div class="darkbg section" style="padding-top:30px; padding-bottom:30px; background-color:black; background-image:url({{asset('frontend/images/home/background18.jpg')}}); background-repeat:no-repeat; background-position:center top; background-size: auto 100%;">
    <div class="section_wrapper mcb-section-inner clearfix">
        <div class="items_group clearfix">
            <!-- One full width row-->
            <div class="column one column_column">
                <div class="column_attr aligncenter">
                    <h3 class="title themecolor" style="font-size: 24px;"><b>@lang('home.test-intro') <span style="font-size: 35px;">D</span>AK<span style="font-size: 35px;">M</span>ARK</b></h3>
                </div>
            </div>
            <!-- One full width row-->
            <div class="column one column_testimonials">
                <div class="testimonials_slider single-photo">
                    <ul class="testimonials_slider_ul">
                        <li>
                            <div class="bq_wrapper">
                                <blockquote>
                                    @lang('home.test1')
                                </blockquote>
                            </div>
                            <div class="author">
                                <h5><a target="_blank" href="#">Tùng</a></h5><span class="company">@lang('home.role1')</span>
                            </div>
                        </li>
                        <li>
                            <div class="bq_wrapper">
                                <blockquote>
                                    @lang('home.test2')
                                </blockquote>
                            </div>
                            <div class="author">
                                <h5><a target="_blank" href="#">Hiếu</a></h5><span class="company">@lang('home.role2')</span>
                            </div>
                        </li>
                        <li>
                            <div class="bq_wrapper">
                                <blockquote>
                                    @lang('home.test3')
                                </blockquote>
                            </div>
                            <div class="author">
                                <h5><a target="_blank" href="#">Tuấn</a></h5><span class="company">@lang('home.role3')</span>
                            </div>
                        </li>
                    </ul>
                    <ul class="slider_pager slider_pagination"></ul>
                </div>
            </div>
        </div>
    </div>
</div>